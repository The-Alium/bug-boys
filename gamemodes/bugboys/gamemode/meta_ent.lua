...
function TTGEnt:GetIfOnGround( height, filter )
    ...
    tracedata.start = pos
    tracedata.endpos = pos - (vector_up * height)

    trace_filter[ 1 ] = self
    trace_filter[ 2 ] = filter
    ...
    if trace_result.HitNonWorld then
        local ent = trace_result.Entity
        return CheckIfInEntTable( ent ) and ent:GetIfStatic()
    elseif trace_result.HitWorld then
        return true
    end
...
function TTGEnt:SetEntTeamForClient( teamnum )
    self:SetNWInt( "IntTeam", teamnum or self.BBTeam )
end
...
function TTGEnt:IsProjectile()
    if CheckIfInEntTable( self ) ~= true then
        return false
    end

    local entref = EntReference( self:GetClass() )
    return entref ~= nil and entref.is_projectile == true
end
