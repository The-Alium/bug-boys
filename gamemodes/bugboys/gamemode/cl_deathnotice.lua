...
local function DrawDeath( x, y, death, hud_deathnotice_time_val )
    local w, h = killicon.GetSize( death.icon )

    local fadeout = (death.time + hud_deathnotice_time_val) - CurTime()
    ...
    -- Draw Icon
    if killicon.Exists( death.icon ) then
        killicon.Draw( x, y, death.icon, alpha )
    else
        killicon.Draw( x, y, "default", alpha )
    end
