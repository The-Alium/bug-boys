...
resource.AddWorkshop( "1808532766" )
GameRestart()
...
local function Check_TimeChangeLevel()
    --3600 = 1 hr
...
function GM:PlayerDisconnected( pl )
    PrintMessage( 2, string.format( "%s disconnected from the gamenot ", pl:Nick() ) )
...
net.Receive( "autoteambalance", function( len, ply )
    if ply:IsSuperAdmin() then
        RunConsoleCommand( "bb_autoteambalance", math.Round( net.ReadFloat() ) )
    end
end )
