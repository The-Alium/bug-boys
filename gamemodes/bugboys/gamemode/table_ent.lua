...
function CheckIfInEntTable( ent )
    return ENT_TABLE[ ent:GetClass() ] ~= nil
end

local function GoDeeper( tbl, x )
    local version_input = x or "none"
...
function EntReference( entstring, x )
    local ent = ENT_TABLE[ entstring ]
    if ent == nil then
        return nil
    end

    if ent.multimode then
        return GoDeeper( ent, x )
    end
...
function ConvertToPrintName_Ent( entname ) -- returns the print name of the input tool name
    local ent = ENT_TABLE[ entname ]
    if ent == nil then
        return "invalid tool name"
    end

    return ent.print_name
end
