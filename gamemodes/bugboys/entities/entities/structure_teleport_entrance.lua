...
function ENT:AttemptTeleport( ply )
    ...
    if not IsValid( self.ExitEnt ) then
        ply:ChatPrint( "Exit not setup for this teleportnot " )
        return false
    end

    if self.ExitEnt.Disabled == true then
        ply:ChatPrint( "Exit is being held a player right nownot " )
        return false
    end
    ...
    ply:ChatPrint( "You must stand closer to the teleportnot " )
    return false
end
