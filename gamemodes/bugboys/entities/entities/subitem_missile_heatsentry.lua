AddCSLuaFile( "subitem_missile_heatsentry.lua" )
...
function ENT:PhysicsCollide( data, phys )
    ...
    if data.HitEntity:IsValidPuck() then -- or CheckIfInEntTable( data.HitEntity ) then
...
function ENT:StartEffect( hitent )
    ...
    for k, ent in pairs( orgin_ents ) do
        if ent.BBTeam ~= self.BBTeam and ent:IsValidPuck() and ent ~= hitent then
            HurtEnt( ent )
        end
    end
