AddCSLuaFile( "subitem_missile_destroyer.lua" )
...
function ENT:StartEffect( hitent )
    ...
    local function HurtEnt( ent )
        --deal 1/5 damage to bug brain
        if ent:GetClass() == "structure_bugbrain" then
...
function ENT:KnockBackStuff()
    ...
    for k, ent in pairs( orgin_ents ) do
        ...
        if ent:GetPhysicsObject():IsValid() and ent:IsValidPuck() and ent.BBTeam ~= self.BBTeam then -- and CheckIfInEntTable( ent ) then
